...
use super::{Coord, VoxelCoord, canonicalize};
...
pub fn raycast<F: FnMut(VoxelCoord) -> bool>(start: Coord,
                                             direction: Coord,
                                             min: VoxelCoord,
                                             max: VoxelCoord,
                                             mut f: F) -> (f32, VoxelCoord) {
    ...
    let VoxelCoord { mut x, mut y, mut z } = start_voxel;
    ...
    let lim_x = if step_x > 0 { max.x } else { min.x };
    let lim_y = if step_y > 0 { max.y } else { min.y };
    let lim_z = if step_z > 0 { max.z } else { min.z };

    // floating point coordinates
    // t_max_c: multiple of direction to get to that edge of voxel
    // t_dc: multiple of direction to move 1 voxel
    let Coord { x: dx, y: dy, z: dz } = direction;
...
mod tests {
    ...
    const MIN: VoxelCoord = VoxelCoord { x: -20, y: -20, z: -20 };
    const MAX: VoxelCoord = VoxelCoord { x: 20, y: 20, z: 20 };
